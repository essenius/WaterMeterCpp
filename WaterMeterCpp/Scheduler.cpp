...
Scheduler::Scheduler(EventServer* eventServer, BatchWriter* measureWriter, BatchWriter* resultWriter) : EventClient(
    "Scheduler", eventServer) {
...
int Scheduler::optimalWriteCount(int round) {
    ...
    for (int i = 0; i <= round - 1; i++) {
        result += 1.0f / static_cast<float>(_writer[i]->getFlushRate());
    }
...
bool Scheduler::processOutput() {
    ...
    for (int i = 0; i < WRITER_COUNT; i++) {
        ...
        const bool needsFlush = _writer[i]->needsFlush();
