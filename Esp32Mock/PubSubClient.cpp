...
bool PubSubClient::publish(const char* topic, const char* payload, bool retain) {
    if (strlen(topic) + strlen(_topic) > static_cast<size_t>(TopicSize) - 1) return false;
    SafeCString::strcat(_topic, topic);
    SafeCString::strcat(_topic, "\n");
    if (strlen(payload) + strlen(_payload) > static_cast<size_t>(PayloadSize) - 1) return false;
