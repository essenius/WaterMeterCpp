...
namespace WaterMeterCppTest {
	...
	QuadraticEllipse fromCoordinates(const Coordinate& center, const Coordinate& radius, const Angle& angle) {
	...
	void assertEllipse(const Coordinate& center, const Coordinate& radius, const Angle& angle) {
		auto coefficient = fromCoordinates(center, radius, angle);
	...
	TEST(QuadraticEllipseTest, ImplicitRadiusTest) {
		auto ellipse = fromCoordinates(Coordinate{ 0, 0 }, Coordinate{ 1, 2 }, Angle{ M_PI / 3 });
		const auto angle = ellipse.getAngle();
		const auto radius = ellipse.getRadius();
		assertAnglesEqual(Angle{ 5 * M_PI / 6 }, angle, "Angle");
