...
namespace WaterMeterCppTest {
    ...
    TEST(MagnetoSensorReaderTest, magnetoSensorReaderRecoverTest) {
        ...
        // First initialize with the null sensor to force an alert
        MagnetoSensor* list[] = {&nullSensor};
        MagnetoSensorReader sensorReader(&eventServer);
        EXPECT_FALSE(sensorReader.begin(list, 1)) << "Reader did not find sensor";
        EXPECT_EQ(1, client.getCallCount()) << "Alert started";
        // Now initalize with the HMC sensor so we get a read value
        list[0] = &sensor;
        Wire.setEndTransmissionTogglePeriod(0);
        Wire.begin();
        EXPECT_TRUE(sensorReader.begin(list, 1)) << "Reader found sensor";
        // force a flatline so the reader starts counting
        Wire.setFlatline(true);
        for (int streak = 0; streak < 5; streak++) {
            sensorReader.read();
        }
        // now get a good value so the counting resets and the alert is stopped
        Wire.setFlatline(false);
        sensorReader.read();
        EXPECT_EQ(2, client.getCallCount()) << "Alert stopped";
    }
